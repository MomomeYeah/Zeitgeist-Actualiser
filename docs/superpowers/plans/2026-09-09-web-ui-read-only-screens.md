...
- TypeScript: `strict: true`, and **no `any`, no non-null `!`, no `as` casts of API data.** The generated types are the contract; a cast is a place the contract was ignored.
...
`assertionStyle: "never"` bans `as` outright, including `as const`. That is stricter than the constraint needs, so where `as const` is genuinely wanted this plan uses `satisfies` instead — see Task 3's token list and Task 9's sentiment ordering.
...
describe("StatusPill", () => {
  ...
  it("names every run status the backend can write", () => {
    // RunStatus is a five-member literal. A status with no label would
    // render as an empty pill, which is the failure mode this catches.
    for (const [status, label] of [
      ...
      const { unmount } = renderWithProviders(<StatusPill status={status} />);
